...
* **Sintesi**: Definisce formalmente gli insiemi ($D, C, A, T$) e le variabili decisionali booleane $x_{d,c,a,t}$. Dimostra la natura NP-hard del problema e l'esplosione combinatoria degli stati. Mostra la costruzione progressiva del grafo di conflitto $G=(V,E)$ evidenziando separatamente le cricche dei docenti (orizzontali) e quelle delle classi (verticali), illustrando poi una soluzione ottima determinata tramite un algoritmo risolutivo in tempo polinomiale. Discute le estensioni List Coloring e ipergrafi. Classifica i vincoli in Hard (ammissibilità fisica) e Soft (qualità didattica), definendo la funzione obiettivo a costi pesati: $\min z = \sum w_i P_i(x)$.
